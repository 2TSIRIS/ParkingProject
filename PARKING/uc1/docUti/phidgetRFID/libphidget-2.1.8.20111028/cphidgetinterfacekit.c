		...
		case PHIDID_INTERFACEKIT_2_2_2:
				...
				|| (phid->phid.deviceIDSpec == PHIDID_INTERFACEKIT_2_2_2))
			{
				...
				for (i = 0; i<phid->phid.attr.ifkit.numSensors; i++)
				{
					if(overrunCount[i])
					{
						snprintf(error_buffer,sizeof(error_buffer),"Channel %d: %d sample overrun detected.", i, overrunCount[i]);
						if (phid->phid.fptrError)
							phid->phid.fptrError((CPhidgetHandle)phid, phid->phid.fptrErrorptr, EEPHIDGET_OVERRUN, error_buffer);
					}
				}
				if((phid->lastPacketCount >= 0) && ((phid->lastPacketCount+1)&0x03) != packetCount)
				{
					snprintf(error_buffer,sizeof(error_buffer),"One or more data packets were lost");
					if (phid->phid.fptrError)
						phid->phid.fptrError((CPhidgetHandle)phid, phid->phid.fptrErrorptr, EEPHIDGET_PACKETLOST, error_buffer);
				}
				if(overcurrentFlag)
				{
					snprintf(error_buffer,sizeof(error_buffer),"Analog input overcurrent detected.");
					if (phid->phid.fptrError)
