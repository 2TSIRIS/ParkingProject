...
void CCONV CPhidget_free(void *arg)
{
	...
	LOG(PHIDGET_LOG_VERBOSE, "In CPhidget_free 1");

	//Call device specific free function if it exists
	if(phid->fptrFree)
		phid->fptrFree(phid);
	LOG(PHIDGET_LOG_VERBOSE, "In CPhidget_free 2");
	...
	if (phid->CPhidgetFHandle) {
		LOG(PHIDGET_LOG_VERBOSE, "In CPhidget_free 3");
		CPhidgetFHandle_free(phid->CPhidgetFHandle); phid->CPhidgetFHandle = NULL;
		LOG(PHIDGET_LOG_VERBOSE, "In CPhidget_free 4");
	}
#endif
	
	LOG(PHIDGET_LOG_VERBOSE, "In CPhidget_free 5");
	...
	LOG(PHIDGET_LOG_VERBOSE, "In CPhidget_free 6");

	free(phid); phid = NULL;
	LOG(PHIDGET_LOG_VERBOSE, "In CPhidget_free 7");
	return;
}
	...
	if(CPhidget_statusFlagIsSet(phid->status, PHIDGET_REMOTE_FLAG))
	{
		...
		if(CPhidget_statusFlagIsSet(phid->status, PHIDGET_SERVER_CONNECTED_FLAG))
		{
			...
			else if(phid->specificDevice == PHIDGETOPEN_LABEL)
			{
				...
				snprintf(key, sizeof(key), "/PCK/Client/%s/%d/%s/-1/%s", inet_ntoa(name_in->sin_addr), (int)name_in->sin_port, 
...
int findActiveDevice(CPhidgetHandle attachedDevice)
{
	...
	for (activeDevice=matches; activeDevice; activeDevice = activeDevice->next)
	{
		...
		result = attachActiveDevice(activeDevice->phid, attachedDevice);
